...
NND.init = function(app){
	if(!_pool)
		_pool = require('./dao-pool').createRethinkdbPool(app);
};
		...
		switch(crudKeywords) {
			...
			case "select":
				...
				if (!tableName || !conditionKey || !conditionValue) {
					console.error('[queryErr] invalid query %j with args %j. the delimeter must be exactly ONE space.', sql, args);
					console.log('tableName = %j, conditionKey = %j, conditionValue = %j', tableName, conditionKey, conditionValue);
					return;
				}
